...
print("\n" + "="*60)
print("STEP 1: Load Township Data & Dissolve to Counties")
print("="*60)

# Load township boundaries
try:
    townships = gpd.read_file('TOWN_MOI_1120317.shp')
    print(f"✅ Loaded {len(townships)} townships from shapefile")
...
if county_col:
    ...
    print(f"\nCounties created:")
    # After dissolve, the grouping column becomes the index
    print(counties.head())
    ...
    plt.tight_layout()
    plt.savefig('township_to_county_dissolve.png', dpi=150, bbox_inches='tight')
    plt.show()
    print("✅ Saved dissolve comparison plot as 'township_to_county_dissolve.png'")
else:
...
# Step 2: Create Synthetic Shelter Data for Hualien County
print("\n" + "="*60)
print("STEP 2: Create Synthetic Shelter Data for Hualien County")
print("="*60)

# Filter Hualien County townships
if county_col and county_col in townships.columns:
    # Try different encodings for Hualien
    hualien_names = ['花蓮縣', '花蓮縣'.encode('utf-8').decode('utf-8')]
    hualien_towns = None
    
    for name in hualien_names:
        if name in townships[county_col].values:
            hualien_towns = townships[townships[county_col] == name].copy()
            break
    
    if hualien_towns is not None and len(hualien_towns) > 0:
        print(f"✅ Found {len(hualien_towns)} townships in Hualien County")
    else:
        print("⚠️ Hualien County not found with expected name. Using sample towns...")
        hualien_towns = townships.head(4).copy()
        # Set sample town names
        sample_names = ['花蓮市', '吉安鄉', '壽豐鄉', '光復鄉']
        for i, (idx, town) in enumerate(hualien_towns.iterrows()):
            hualien_towns.loc[idx, 'TOWNNAME'] = sample_names[i]
            hualien_towns.loc[idx, county_col] = '花蓮縣'
else:
...
plt.savefig('hualien_shelters_distribution.png', dpi=150, bbox_inches='tight')
plt.show()
print("✅ Saved shelter distribution plot as 'hualien_shelters_distribution.png'")

# Step 3: Groupby — Count Shelters & Sum Capacity per Town
print("\n" + "="*60)
print("STEP 3: Groupby — Count Shelters & Sum Capacity per Town")
print("="*60)
...
plt.tight_layout()
plt.savefig('town_shelter_statistics.png', dpi=150, bbox_inches='tight')
plt.show()
print("✅ Saved statistics plots as 'town_shelter_statistics.png'")

# Step 4: Merge Stats Back to Geometry → Choropleth Map
print("\n" + "="*60)
print("STEP 4: Merge Stats Back to Geometry → Choropleth Map")
print("="*60)
...
townships_with_stats['town_name'] = townships_with_stats.get(town_col, [f'Town_{i}' for i in range(len(hualien_towns))])
...
plt.tight_layout()
plt.savefig('choropleth_maps.png', dpi=150, bbox_inches='tight')
plt.show()
print("✅ Saved choropleth maps as 'choropleth_maps.png'")
...
print(f"\n📁 Generated Files:")
print("- township_to_county_dissolve.png")
print("- hualien_shelters_distribution.png") 
print("- town_shelter_statistics.png")
print("- choropleth_maps.png")

print(f"\n🎓 Key Concepts Learned:")
print("1. Dissolve Operation - Merges geometries based on attribute values")
print("2. Groupby Operations - Statistical aggregation with spatial data")
print("3. Spatial Joins & Merges - Combining tabular data with geometries")
print("4. Choropleth Mapping - Color-coded spatial visualization")

print(f"\n✅ Lab 1 Complete! All visualizations saved as PNG files.")
print("🚀 Ready for Raster Analysis in next lab!")
