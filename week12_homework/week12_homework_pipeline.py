...
- 有效像元：{data['metadata']['valid_pixels']:,} pixels（{data['metadata']['valid_percent']:.1f}%）；已套用 SCL cloud/shadow/snow mask，排除 SCL {data['metadata']['scl_masked_values']} 與異常反射率像元。
...
OOB accuracy ({rf_metrics['oob_score']:.3f}) 與 test accuracy ({rf_metrics['test_accuracy']:.3f}) 差距僅 {abs(rf_metrics['oob_test_gap']):.3f}，Macro/Weighted F1 gap 為 {rf_metrics['macro_weighted_gap']:.3f}，低於 0.03；各類 test support 皆大於 30 pixels，因此內部驗證相對穩定。外部 SWCB 驗證顯著較低，顯示模型在 ROI 類別內表現良好，但「裸地/崩塌」對官方崩塌清冊的語意與尺度轉換仍不足。FN 多半會集中在小面積或窄長邊坡崩塌、山區陰影及雲/陰影遮罩附近，這些位置在 20 m Sentinel-2 影像中容易被森林或裸地背景稀釋。
